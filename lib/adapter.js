...
module.exports = (function() {
  ...
  var adapter = {
    ...
    /**
     * This calls the collection.create from arangojs.
     * This creates the edge collection.
     * if conflict of name is returned 409, that collection is already created and does
     * not create but returns normally
     *
     * edgeCollectionName: name of the edge collection
     * attributes [optionnal]: data to include in the edge
     * cb
     */
    createEdgeColl: function(connectionName, edgeCollectionName, attributes, cb){
      var db = connections[connectionName].db;
      if (cb === undefined){
        cb = attributes;
        attributes = {};
      ...
      var collection = db.edgeCollection(edgeCollectionName);
      collection.create({waitForSync: true}, function(e, r){
        if (e){
        ...
        console.log(edgeCollectionName + ' now exists');
        return cb(null, r);
      });
    },

    /**
     * This calls the edgeCollection.save from arangojs.
     * This updates the edge collection and creates a new edge from id1 to id2.
     *
     * edgeCollectionName: name of the edge collection
     ...
     * cb
     */
    deleteAllEdges: function(connectionName, collectionName, edgeCollectionName, id, attributes, cb){
      var db = connections[connectionName].db;
      if (cb === undefined){
        cb = attributes;
        attributes = {};
      ...
      var collection = db.edgeCollection(edgeCollectionName);
      var removedkeys = [];
      collection.edges(id, function(err, data){
        data.forEach(function(elem){
          collection.remove(elem['_key'], function(e, r){
            if (e){
              return cb(e);
            }
            removedkeys.push(r);
        ...
        return cb(null, removedkeys); // @TODO: Return something usefull if possible
      });
    },

    deleteEdge: function(connectionName, collectionName, edgeCollectionName, id, attributes, cb){
      var db = connections[connectionName].db;
      if (cb === undefined){
        cb = attributes;
        attributes = {};
    ...
    dropColl: function(connectionName, collectionName, data, cb) {
      var col = connections[connectionName].db.collection(collectionName);

      return col.drop(cb);
    },
    truncateColl: function(connectionName, collectionName, data, cb) {
      var col = connections[connectionName].db.collection(collectionName);

      return col.truncate(cb);
    },
    ...
    update: function(connectionName, collectionName, searchCriteria , values , cb) {
      ...
      var qupdate = 'UPDATE { _key: x._key, ' +
          this._queryFormatter(values, ':', undefined, true) + ' } IN ' + collectionName;
    ...
    deleteEdges: function(connectionName, from, to, options, cb) {
      return connections[connectionName].deleteEdges(from, to, options, cb);
    },
     ...
     */
    _queryFormatter: function(searchCriteria, separator, begin, update) {
      var r = '';
      var user = {};
      var sep = separator;
      var sep2;
      for (var k in searchCriteria) {
        ...
        if (searchCriteria.hasOwnProperty(k)) {
          ...
          console.log(v_override);
          if(typeof v_override === 'object' && (k === '>=' || k === '<=')){
            sep = (Object.keys(v_override)[0]) ? Object.keys(v_override)[0] : sep;
            sep2 = Object.keys(v_override)[1];
            //console.log(sep);
            //console.log('sdasdasd');
          }
          // Datetime to arangodb format
          if (v_override.constructor == Date){
            v_override = v_override.toISOString();
          }
          if(v_override instanceof Array && !update){
            var newseperator = ' IN ';
            r += begin + k_override + newseperator + JSON.stringify(v_override);
          } else {
            var secondSeperator = (sep2) ? ' AND ' + begin + k_override + sep2 + this._queryParamWrapper(v_override, sep2) : '';
            r += begin + k_override + sep + this._queryParamWrapper(v_override, sep) + secondSeperator;
          }
    ...
    _queryParamWrapper: function(param, sep) {
      if (typeof param == 'string'){
        return "'" + param + "'"
      }
      if(typeof param === 'object' && sep !== ':'){
        return "'" + param[sep] + "'";
      }
