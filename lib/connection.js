...
module.exports = (function() {
  ...
  DbHelper.prototype.registerCollections = function() {
    ...
    async.auto({
          ...
          createMissingCollections: ['getCollections', function(next, results) {
            ...
            var missingCollectionsNames = _.map(missingCollections, function(obj) {
              return obj.adapter.collection;
            });
            console.log('currentCollections::', currentCollections)
            console.log('missingCollections::', missingCollectionsNames)
  ...
  DbHelper.prototype.update = function(collection, options, values, cb) {
    ...
    this.db.query(qb.toAQL() + ' LET modified = NEW RETURN modified', function(err, cursor) {
      console.log('after successful update');
      console.log(err);
      console.log(cursor);
      if (err) return cb(err);
      cursor.all(function(err, vals) {
  ...
  DbHelper.prototype.join = function(collection, criteria, cb) {
    ...
    var qb = optionsToQuery(collection, criteria).toAQL(),
        ...
        ret = ' RETURN { "d" : d';
    var cjoins = [];
    criteria.joins.forEach(function(elem){
      if(elem.parent.indexOf('__') === -1){
        cjoins.push(elem);
      }
    });

    //console.log('THIS IS THE EDGE IN JOIN');
    var currentEdges = this.edgeCollections;
    _.each(cjoins, function(join) {
      var edges = currentEdges.filter(function(elem){
        return (elem.from === join.parent && elem.collection === join.alias) ||
            (elem.from === join.parent && elem.model === join.alias);
      });
      var edgesObj = {};
      edgesObj.parent = (edges[0]) ? edges[0].from : join.parent;
      edgesObj.edge = (edges[0]) ? edges[0].edge : join.alias + 'Of';
      edgesObj.alias = (edges[0]) ? ((edges[0].collection) ? edges[0].collection : edges[0].model) : join.alias;
      ret += ', "' + edgesObj.alias + '" : (FOR ' + edgesObj.alias + ' IN NEIGHBORS(' +
          aqb.str(edgesObj.parent).toAQL() + ',' + aqb.str(edgesObj.edge).toAQL() + ',' +
          'd._id' + ', "any", [], ' +
          '{"includeData": true}) RETURN ' + edgesObj.alias + ')';
    });
    ret += ' }';
    console.log(qb + ret);
